...
### SBS KEN DEBUG ###
// if(!empty($_POST['path_th'])) {
//     $path = $base . $_POST['path_th'];
// } else {
...
if(isset($_POST['paths'])){
    ...
    ### SBS KEN DEBUG ###
    // if(!empty($_POST['path_th'])) {
    //     list($path,$path_thumb,$name) = returnPaths($_POST['path_th'],$name,$config);
    // } else {
...
if (isset($info['extension']) && !(isset($_GET['action']) && $_GET['action'] == 'delete_folder') &&
    ...
    if($_GET['action'] != 'create_folder' && $_GET['action'] != 'rename_folder' && $_GET['action'] != 'rename_file' && $_GET['action'] != 'duplicate_file') {
