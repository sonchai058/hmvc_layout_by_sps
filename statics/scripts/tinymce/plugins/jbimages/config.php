...
$root_path = explode('/', $_SERVER['PHP_SELF']);
$path_upload = $_SERVER['DOCUMENT_ROOT'] . '/' . APP_HOST_ADDRESS . '/file_upload/module/jbimages';

if (!empty($root_path[1]) && $root_path[1] != 'shared_view') {
    $path_upload = $_SERVER['DOCUMENT_ROOT'] . '/' . $root_path[1] . '/file_upload/module/jbimages';
} else {
    $path_upload = $_SERVER['DOCUMENT_ROOT'] . '/file_upload/module/jbimages';

}

$config['img_path'] = APP_HOST_ADDRESS . 'file_upload/module/jbimages'; // Relative to domain name
$config['upload_path'] = $path_upload; // Physical path. [Usually works fine like this]
