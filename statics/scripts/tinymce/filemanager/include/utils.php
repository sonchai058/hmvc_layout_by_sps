...
function deleteFile($path, $path_thumb, $config)
{
    if ($config['delete_files']) {
        ...
        if ($ftp) {
            ...
            ### SBS KEN DEBUG แก้ลบไฟล์ภาษาไทย ###
            // if (!empty($path)) {
            //     $path = explode('/', $path);
            //     $setPath = array();
            //     foreach ($path as $key => $value) {
            //         if (mb_detect_encoding($value) != 'ASCII') {
            //             $value = (iconv("UTF-8", "windows-874//IGNORE", $value));
            //         }

            //         $setPath[] = $value;
            //     }

            //     $path = implode('/', $setPath);
            // }

            // if (!empty($path_thumb)) {
            //     $path_thumb = explode('/', $path_thumb);
            //     $setPath = array();
            //     foreach ($path_thumb as $key => $value) {
            //         if (mb_detect_encoding($value) != 'ASCII') {
            //             $value = (iconv("UTF-8", "windows-874//IGNORE", $value));
            //         }
...
function rename_file($old_path, $name, $ftp = null, $config = null)
{
    ...
    // $name = (iconv("UTF-8", "windows-874//IGNORE", $name));
    $info = pathinfo($old_path);
    ### SBS KEN DEBUG แก้เปลี่ยนชื่อไฟล์ภาษาไทย ###
    // if (!empty($info['dirname'])) {
    //     $info['dirname'] = explode('/', $info['dirname']);
    //     $setPath = array();
    //     foreach ($info['dirname'] as $key => $value) {
    //         if (mb_detect_encoding($value) != 'ASCII') {
    //             $value = (iconv("UTF-8", "windows-874//IGNORE", $value));
    //         }
...
function create_folder($path = null, $path_thumbs = null, $ftp = null, $config = null)
{
    if ($ftp) {
        ...
        ### SBS KEN DEBUG แก้เปลี่ยนชื่อไฟล์ภาษาไทย ###
//         $pathSet = explode('\\', $path);
//         if (!empty($pathSet) && !empty($pathSet[1])) {
//             $path = explode('/', $pathSet[0]);
//             $setPath = array();
//             foreach ($path as $key => $value) {
//                 if (mb_detect_encoding($value) != 'ASCII') {
...
//             $path = implode('/', $setPath);
//             $pathSet[1] = (iconv("UTF-8", "windows-874//IGNORE", $pathSet[1]));
//             $path = $path . '\\' . $pathSet[1];
//         }

//         $pathThumbSet = explode('\\', $path_thumbs);
//         if (!empty($pathThumbSet) && !empty($pathThumbSet[1])) {
//             $path_thumbs = explode('/', $pathThumbSet[0]);
//             $setPathThumb = array();
//             foreach ($path_thumbs as $key => $value) {
//                 if (mb_detect_encoding($value) != 'ASCII') {
// //                      $value = (mb_convert_encoding($value, 'UTF-8', mb_detect_encoding($value)));
//                     $value = (iconv("UTF-8", "windows-874//IGNORE", $value));
//                 }
...
//                 $setPathThumb[] = $value;
//             }
//             $path_thumbs = implode('/', $setPathThumb);
//             $pathThumbSet[1] = (iconv("UTF-8", "windows-874", $pathThumbSet[1]));
//             $path_thumbs = $path_thumbs . '\\' . $pathThumbSet[1];
//         }
        ### SBS KEN DEBUG แก้เปลี่ยนชื่อไฟล์ภาษาไทย ###
        ...
        // if (isset($_POST['path_th'])) {
            if ($path && !file_exists($path)) {
                mkdir($path, $permission, true) or die("$path cannot be found");
